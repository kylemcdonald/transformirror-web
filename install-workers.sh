...
start_without_systemd() {
    if command -v tmux >/dev/null 2>&1; then
        ...
        tmux new-session -d -s "$SERVICE_ID" "cd '$WORKDIR' && env HF_HOME=/workspace/.cache PYTHONUNBUFFERED=1 TRANSFORMIRROR_USE_STABLE_FAST=1 TRANSFORMIRROR_MAX_WORKERS=1 '$WORKDIR/run-workers.sh' >>'$LOGDIR/workers.log' 2>&1"
...
Environment=TRANSFORMIRROR_MAX_WORKERS=1
[Install]
